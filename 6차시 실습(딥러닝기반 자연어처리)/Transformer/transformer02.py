...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        pe[:, 0::2] = torch.sin(position * div_term)   # 짝수 (position * div_term: (10, 8))
        pe[:, 1::2] = torch.cos(position * div_term)   # 홀수

        pe = pe.unsqueeze(0)   # (1, L, D)  → batch 차원 추가
        self.register_buffer("pe", pe)  # 학습 파라미터는 아니지만, 모델과 함께 저장되도록
...
def train():
    ...
    model.eval()
    with torch.no_grad():
        src, trg_input, trg_output, src_key_padding_mask, tgt_key_padding_mask = next(iter(valid_loader))
